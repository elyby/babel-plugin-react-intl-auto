...
const replaceProperties = (properties, state, exportName) => {
  ...
  for (const prop of properties) {
    ...
    if (objectValuePath.isObjectExpression()) {
      let objProps = objectValuePath.get('properties'); // { id: 'already has id', defaultMessage: 'hello' }
      ...
      if (!state.opts.removeDefaultMessage) {
        messageDescriptorProperties.push((0, _utils.objectProperty)('defaultMessage', objectValuePath.node));
      }
    } else {
      ...
      if (evaluated.confident && typeof evaluated.value === 'string') {
        ...
        if (!state.opts.removeDefaultMessage) {
          messageDescriptorProperties.push((0, _utils.objectProperty)('defaultMessage', evaluated.value));
        }
      }
    }
